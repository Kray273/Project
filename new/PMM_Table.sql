...
COMMENT ON COLUMN community.um_no IS '회원번호';
...
/* 댓글 */
CREATE TABLE reply (
	r_no VARCHAR2(50) NOT NULL, /* 일렬번호 */
...
COMMENT ON COLUMN reply.r_no IS '일렬번호';

COMMENT ON COLUMN reply.cm_bdno IS '게시물번호';

COMMENT ON COLUMN reply.um_no IS '회원번호';
...
COMMENT ON COLUMN buy.d_dno IS '배송업처번호';
...
CREATE TABLE askReview (
	ar_no VARCHAR2(50) NOT NULL, /* 답변번호 */
